...
def skapa_tank():
    ...
    # Cylinder
    fig.add_trace(go.Surface(x=x_cyl, y=y_cyl, z=z_cyl, colorscale='Blues', opacity=0.7, name='Cylinderdel'))
    ...
    fig.add_trace(go.Surface(x=x_bot, y=y_bot, z=z_bot, colorscale='Reds', opacity=0.7, name='Botten'))

    # Topp
    z_top = np.linspace(h_botten+h_cylinder, h_botten+h_cylinder+h_topp, 20)
    ...
    if topp_form == 'Platt':
        ...
        x_top = r * (1 - (z_top-(h_botten+h_cylinder))/h_topp) * np.cos(theta_top)
        y_top = r * (1 - (z_top-(h_botten+h_cylinder))/h_topp) * np.sin(theta_top)
    elif topp_form == 'Sfärisk':
        x_top = r * np.sin(np.pi/2 * (z_top-(h_botten+h_cylinder))/h_topp) * np.cos(theta_top)
        y_top = r * np.sin(np.pi/2 * (z_top-(h_botten+h_cylinder))/h_topp) * np.sin(theta_top)
    else:  # Kupol
        x_top = r * np.sqrt((z_top-(h_botten+h_cylinder))/h_topp) * np.cos(theta_top)
        y_top = r * np.sqrt((z_top-(h_botten+h_cylinder))/h_topp) * np.sin(theta_top)
    fig.add_trace(go.Surface(x=x_top, y=y_top, z=z_top, colorscale='Greens', opacity=0.7, name='Topp'))
